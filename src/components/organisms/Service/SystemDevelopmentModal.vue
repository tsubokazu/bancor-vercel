...
<template>
  ...
  >
    <div class="flex items-center gap-3">
      ...
      <div class="h-8 w-8">
        <img
          :src="systemDevelopmentService.infoIcon"
          :alt="systemDevelopmentService.name"
          class="h-full object-cover"
        />
      </div>
      <div class="text-[22px] font-bold">
        {{ systemDevelopmentService.outlineTitle }}
      </div>
    </div>
    <!-- 説明 -->
    <div class="text-[17px] leading-[1.8] text-[#64748b]">
      {{ systemDevelopmentService.outlineBody }}
    </div>
    <!-- ヘルプタイトル -->
    <div class="flex items-center gap-3">
      <!-- アイコン -->
      <div class="h-8 w-8">
        <img
          :src="systemDevelopmentService.helpIcon"
          :alt="systemDevelopmentService.name"
          class="h-full object-cover"
        />
      </div>
      <div class="text-[22px] font-bold">
        {{ systemDevelopmentService.helpTitle }}
      </div>
    </div>
    <!-- カード説明 -->
    <div class="flex flex-wrap items-center gap-6">
      <div
        v-for="(item, index) in systemDevelopmentService.cardItems"
        class="relative flex h-[240px] w-[262px] flex-col items-center justify-center gap-4 rounded-[10px] border border-[#cbd5e1] bg-white"
      >
        <div class="absolute top-0 left-0 flex flex-col"></div>
        <div class="h-[120px]">
          <img :src="item.imgUrl" class="h-full object-cover" />
        </div>
        <div class="px-[17px] text-[15px] font-bold">
          {{ item.outline }}
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center justify-end gap-4">
      ...
      <!-- ボタン -->
      <NuxtLink
        :to="systemDevelopmentService.linkUrl"
