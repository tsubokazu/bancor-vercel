<script setup lang="ts">
  ...
  import type { BeautyStoresObject, BupuraShopObject } from '~/types/stores';
  ...
  const foundStore = stores.find((store) => Number(store.topicsId) === storeId);
  if (foundStore === undefined) {
    const currentDomain = window.location.origin;
    window.location.href = `${currentDomain}/stores`;
  }
  const targetStore: BupuraShopObject = foundStore as BupuraShopObject;

  // GoogleMapの中心座標を取得
  const url = targetStore.mapUrl;
...
<template>
  ...
  >
    ...
    <div class="relative flex w-full flex-col items-center">
      ...
      <!-- 店舗情報 -->
      <div
        class="flex w-full flex-col rounded-[40px] bg-white py-[64px] px-[50px] tb:w-[95%] pc:max-w-[1100px]"
      >
        <!-- 店舗写真＆アクセス等 -->
        <div
        ...
        <!-- 予約・口コミ -->
        <div
          class="mb-12 flex w-[95%] flex-col items-center space-y-4 tb:flex-row tb:justify-between tb:space-y-0 pc:max-w-[1100px]"
        >
          <!-- 予約 -->
          <NuxtLink
            :to="targetStore.reserveUrl"
        ...
        >
          ...
          <div>
            <div class="grid grid-cols-2 gap-4 tb:grid-cols-4">
              <AtomsBasicIcon
                v-for="(imgUrl, index) in targetStore.gallery"
                size="h-[193px]"
                :iconUrl="imgUrl"
              ></AtomsBasicIcon>
            </div>
