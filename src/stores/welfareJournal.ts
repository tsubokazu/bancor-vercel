...
export const useWelfareJournalStore = defineStore('welfare-journal', () => {
  // Welfare Journalの記事をKurocoから取得
  ...
  const fetchJournals = async () => {
    ...
    if (!data.value || error.value) {
      ...
      // 取得したデータを型に当てはめる
      journalList.value = journals.value.list.map((journal: any) => ({
      ...
      // 注目記事をpv数でソートしてリスト化
      featureList.value = journalList.value.sort((prev: any, next: any) => {
...
if (import.meta.hot) {
  import.meta.hot.accept(
    acceptHMRUpdate(useWelfareJournalStore, import.meta.hot)
  );
}
